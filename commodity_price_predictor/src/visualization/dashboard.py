...
# Import modules after ensuring directories exist
try:
    ...
    st.info("Please make sure you've run the setup command: `python main.py setup`")
...
def main():
    ...
    if not commodities:
        ...
        if st.button("Generate Sample Data"):
            st.info("Generating sample data for commodities...")
            
            with st.spinner("Generating data..."):
                try:
                    # Generate sample data
                    raw_data = data_collector.fetch_historical_data(period='5y')
                    ...
                    st.success("Data generation completed! Please refresh the page.")
                except Exception as e:
        ...
        for model_name in available_models:
            ...
            if os.path.exists(metrics_file):
                try:
                    ...
                    st.error(f"Error loading metrics for {model_name}: {str(e)}")
